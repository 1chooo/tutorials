...
| Sample | Input | Output |
| :------: | :--------: | :------: |
| 說明 | 第一行有一個正整數 N (1 <= N <= 20)，代表要計算 BMR 的人數。接下來 i 行 （i = 1, 2, …, N），每行各有四個正整數 G、A、H、W，彼此間以空白隔開。正整 數 G：1 (男)或 0 (女)，代表計算者的性別；三個正整數 A (1 <= A<= 150)、H (1 <= H <= 250)、W (1 <= W <= 300)：代表計算者的年齡、身高(cm)、體重(kg)。 | 對於每筆測資輸出一個浮點數，代表該人的基礎代謝率。BMR 四捨五入至 小數點後兩位，印完一人後換行。 |
